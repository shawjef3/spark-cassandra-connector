...
object DsePredicateRules extends CassandraPredicateRules with Logging {
  ...
  private val textTypes = Set[ColumnType[_]](TextType, AsciiType, VarCharType)
  private val numericTypes = Set[ColumnType[_]](IntType, BigIntType, SmallIntType, TinyIntType, FloatType, DoubleType,
    VarIntType, DecimalType, TimestampType)
  // TODO: DATE, INET, TIME, TIMESTAMP, TIMEUUID, UUID
  ...
  override def apply(
    ...
    val pushDownRules = Seq[PredicateRule](
      pushOnlySolrQuery
    ) ++ storageAttachedIndexPredicateRules(predicates, tableDef) // TODO: safety valve
  ...
  // TODO: InClausePredicateRules is applied after DsePredicateRules, it may IN clause from pushdown - we should probably apply SAI later then IN clause
  ...
  private def pushDownPredicates(predicates: AnalyzedPredicates, table: TableDef)
    ...
    val pushedEqualityPredicates = predicates.handledByCassandra.collect {
      case f if PredicateOps.FilterOps.isEqualToPredicate(f) => PredicateOps.FilterOps.columnName(f)
  ...
  private def pushDownSAITextPredicates(predicates: AnalyzedPredicates, table: TableDef): AnalyzedPredicates =
    pushDownPredicates(predicates, table) { filter =>
      FilterOps.isEqualToPredicate(filter) &&
        textTypes.contains(table.columnByName(FilterOps.columnName(filter)).columnType)
    }
  ...
  private def pushDownSAINumericPredicates(predicates: AnalyzedPredicates, table: TableDef): AnalyzedPredicates =
    pushDownPredicates(predicates, table) { filter =>
      (FilterOps.isEqualToPredicate(filter) || FilterOps.isRangePredicate(filter)) &&
        numericTypes.contains(table.columnByName(FilterOps.columnName(filter)).columnType)
    }
